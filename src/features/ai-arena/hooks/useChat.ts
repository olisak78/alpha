...
async function migrateToIndexedDB(): Promise<void> {
  ...
  console.log("[useChat] Starting migration from localStorage to IndexedDB...");

  const legacyConversations = loadLegacyConversations();
  if (legacyConversations.length === 0) {
    localStorage.setItem(MIGRATION_KEY, "true");
    console.log("[useChat] No legacy conversations to migrate");
    return;
  }
  ...
  try {
    ...
    console.log(`[useChat] Successfully migrated ${legacyConversations.length} conversations to IndexedDB`);
  } catch (error) {
