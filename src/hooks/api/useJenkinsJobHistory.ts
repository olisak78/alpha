...
/**
 * Hook to fetch Jenkins job history with pagination
 * 
 ...
 * 
 * @example
 * const { data, isLoading, refetch } = useJenkinsJobHistory(10, 0, true);
 */
export function useJenkinsJobHistory(
  ...
  return useQuery({
    queryKey: queryKeys.selfService.jobHistory(limit, offset, onlyMine),
    queryFn: () => fetchJenkinsJobHistory(limit, offset, onlyMine),
