...
describe('UnifiedLinkCard', () => {
  ...
  describe('Tag search interactions', () => {
    it('sets search term when a tag badge is clicked', async () => {
      render(<UnifiedLinkCard linkData={mockLink} />);
 const tagBadge = screen.getByText('tag1');
      await user.click(tagBadge);
      expect(mockSetSearchTerm).toHaveBeenCalledWith('tag1');
    });

    it('sets search term when pressing Enter on a tag badge', async () => {
      render(<UnifiedLinkCard linkData={mockLink} />);
      const tagBadge = screen.getByText('tag2');
      tagBadge.focus();
      await user.keyboard('{Enter}');
      expect(mockSetSearchTerm).toHaveBeenCalledWith('tag2');
    });
  });

  describe('Category search interactions', () => {
    it('sets search term when category is clicked', async () => {
      mockUseLinkCard.category = { id: 'cat-1', name: 'Development', icon: null as any, color: 'bg-blue-500' };
      render(<UnifiedLinkCard linkData={mockLink} />);
      const categoryButton = screen.getByTitle('Search by category "Development"');
      await user.click(categoryButton);
      expect(mockSetSearchTerm).toHaveBeenCalledWith('Development');
    });

    it('sets search term when pressing Enter on category', async () => {
      mockUseLinkCard.category = { id: 'cat-1', name: 'Development', icon: null as any, color: 'bg-blue-500' };
      render(<UnifiedLinkCard linkData={mockLink} />);
      const categoryButton = screen.getByTitle('Search by category "Development"');
