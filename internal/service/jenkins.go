...
// GetConcourseLatestExecution retrieves the latest build number from concourse-status-developer-portal job
func (s *JenkinsService) GetConcourseLatestExecution(ctx context.Context) (int, error) {
	log := logger.WithContext(ctx)

	log.Info("Getting Concourse latest execution")

	// Use cfsstatus JAAS instance
	tokenName, token, err := s.getJenkinsCredentials(ctx, "cfsstatus")
	if err != nil {
		log.Errorf("Failed to get Jenkins credentials: %v", err)
		return 0, err
	}

	// Build URL to get last successful build
	baseURL := strings.Replace(s.cfg.JenkinsBaseURL, "{jaasName}", "cfsstatus", -1)
	jobURL := fmt.Sprintf("%s/job/concourse-status-developer-portal/lastSuccessfulBuild/api/json", baseURL)
	...
	resp, err := s.httpClient.Do(req)
	if err != nil {
		log.Errorf("Jenkins request failed: %v", err)
		return 0, err
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 400 {
		body, _ := io.ReadAll(resp.Body)
		log.Errorf("Jenkins request failed: status=%d, body=%s", resp.StatusCode, string(body))
		return 0, fmt.Errorf("jenkins request failed: status=%d", resp.StatusCode)
	}

	// Parse response to get build number
	var buildData struct {
		Number int `json:"number"`
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&buildData); err != nil {
		...
		return 0, fmt.Errorf("failed to parse jenkins response: %w", err)
	}

	log.Infof("Latest execution number: %d", buildData.Number)
	return buildData.Number, nil
}

// GetConcourseJobs retrieves the concourse jobs artifact from Jenkins
func (s *JenkinsService) GetConcourseJobs(ctx context.Context, buildNumber int, landscape string) (interface{}, error) {
	log := logger.WithContext(ctx).WithFields(map[string]interface{}{
		"buildNumber": buildNumber,
		"landscape":   landscape,
	})

	log.Info("Getting Concourse jobs")
	...
	// Build URL to artifact
	baseURL := strings.Replace(s.cfg.JenkinsBaseURL, "{jaasName}", "cfsstatus", -1)
	artifactURL := fmt.Sprintf("%s/job/concourse-status-developer-portal/%d/artifact/concourse_jobs.json", baseURL, buildNumber)

	log.Infof("Fetching Concourse jobs from: %s", artifactURL)
	...
	resp, err := s.httpClient.Do(req)
	if err != nil {
		log.Errorf("Jenkins artifact request failed: %v", err)
		return nil, err
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 400 {
		body, _ := io.ReadAll(resp.Body)
		log.Errorf("Jenkins artifact request failed: status=%d, body=%s", resp.StatusCode, string(body))
		return nil, fmt.Errorf("jenkins artifact request failed: status=%d", resp.StatusCode)
	}

	// Parse the JSON response
	var allJobs map[string][]interface{}
	if err := json.NewDecoder(resp.Body).Decode(&allJobs); err != nil {
		log.Errorf("Failed to parse concourse jobs: %v", err)
		return nil, fmt.Errorf("failed to parse concourse jobs: %w", err)
	}

	// If landscape is specified, filter to that landscape
	if landscape != "" && landscape != "all" {
		if jobs, exists := allJobs[landscape]; exists {
			log.Infof("Returning %d jobs for landscape: %s", len(jobs), landscape)
			return map[string][]interface{}{landscape: jobs}, nil
		}
		log.Warnf("No jobs found for landscape: %s", landscape)
		return map[string][]interface{}{}, nil
	}

	log.Infof("Returning all jobs for %d landscapes", len(allJobs))
	return allJobs, nil
}
