...
// CreateDocumentationRequest represents the payload for creating documentation
...
// UpdateDocumentationRequest represents the payload for updating documentation
...
// CreateDocumentation validates and creates new documentation
func (s *DocumentationService) CreateDocumentation(req *CreateDocumentationRequest) (*DocumentationResponse, error) {
	...
	owner, repo, branch, docsPath, provider, err := s.parseGitHubURL(req.URL)
	if err != nil {
...
// GetDocumentationByID retrieves documentation by ID
...
// parseGitHubURL parses a GitHub URL and extracts an owner, repo, branch, docs path, and provider
// Supports two formats:
// 1. Full path: https://github.tools.sap/{owner}/{repo}/tree/{branch}/{path}
// 2. Repository root: https://github.tools.sap/{owner}/{repo} (defaults to the main branch and root path)
...
func (s *DocumentationService) parseGitHubURL(urlStr string) (owner, repo, branch, docsPath, provider string, err error) {
	// Parse URL
	...
	// Get all configured providers with their base URLs and validate host
	providersMap := s.authService.GetAllProviders()
	// Always allow GitHub.com as a valid provider
	providersMap["github.com"] = "https://github.com"

	var validProvider string
	for providerName, baseURL := range providersMap {
		// Parse base URL to extract host for comparison
		if baseURL != "" {
			if parsedBase, err := url.Parse(baseURL); err == nil && parsedBase.Host == u.Host {
	...
	if len(parts) == 2 {
		// Default to the main branch and root path for repository root URLs
		branch = "main"
		docsPath = "/"
	} else if len(parts) >= 5 {
	...
	return owner, repo, branch, docsPath, validProvider, nil
}
