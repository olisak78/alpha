...
func (suite *AICoreServiceTestSuite) TestCreateDeployment_WithConfigurationRequest_Success() {
	...
	responses := map[string]mockResponse{
		...
		"GET:/v2/lm/configurations/config-456": {
			StatusCode: 200,
			Body: `{
				"id": "config-456",
				"name": "my-llm-config",
...
func (suite *AICoreServiceTestSuite) TestCreateDeployment_ConfigurationCreationFails_Error() {
	...
	// Setup mocks - CreateDeployment calls CreateConfiguration internally, which calls getUserTeam
	// CreateConfiguration calls getUserTeam once, then CreateDeployment tries to call getUserTeam again
	// So we need to expect 2 calls total
	suite.userRepo.EXPECT().GetByEmail(email).Return(member, nil).Times(2)
	suite.teamRepo.EXPECT().GetByID(teamID).Return(team, nil).Times(2)
...
func (suite *AICoreServiceTestSuite) TestUpdateDeployment_APIError_Error() {
	...
	suite.Error(err)
	suite.Nil(result)
	suite.Contains(err.Error(), "deployment not found")
}
...
// Tests for team validation logic through public methods
// Note: getUserAccessibleTeams is unexported, so we test the team validation logic
// through the public methods that use it (CreateConfiguration and CreateDeployment)

// Tests for team validation logic in CreateConfiguration

func (suite *AICoreServiceTestSuite) TestCreateConfiguration_WithValidTeam_Success() {
	// Setup
	email := "team.member@example.com"
	teamID := uuid.New()
	...
	configRequest := &service.AICoreConfigurationRequest{
		...
		ExecutableID: "azure-openai",
		ScenarioID:   "foundation-models",
		Team:         "team-alpha", // Specify team that user has access to
	}
	...
	// Setup mocks - getUserAccessibleTeams is called to validate team access
	suite.userRepo.EXPECT().GetByEmail(email).Return(member, nil).Times(1) // Once for validation
	suite.teamRepo.EXPECT().GetByID(teamID).Return(team, nil).Times(1)
...
func (suite *AICoreServiceTestSuite) TestCreateConfiguration_WithInvalidTeam_Error() {
	// Setup
	email := "team.member@example.com"
	teamID := uuid.New()
	...
	configRequest := &service.AICoreConfigurationRequest{
		...
		ExecutableID: "azure-openai",
		ScenarioID:   "foundation-models",
		Team:         "team-beta", // Specify team that user does NOT have access to
	}
	...
	suite.Error(err)
	suite.Nil(result)
	suite.Contains(err.Error(), "user does not have access to team: team-beta")
}

func (suite *AICoreServiceTestSuite) TestCreateConfiguration_TeamValidationFails_Error() {
	// Setup
	email := "nonexistent@example.com"
	...
	// Setup mocks - getUserAccessibleTeams fails
	suite.userRepo.EXPECT().GetByEmail(email).Return((*models.User)(nil), errors.ErrUserNotFound)
...
func (suite *AICoreServiceTestSuite) TestCreateConfiguration_NoTeamSpecified_UsesUserTeam_Success() {
	// Setup
	email := "team.member@example.com"
	teamID := uuid.New()
	...
	configRequest := &service.AICoreConfigurationRequest{
		...
		ExecutableID: "azure-openai",
		ScenarioID:   "foundation-models",
		// No Team specified - should use user's assigned team
	}
	...
	// Setup mocks - getUserTeam is called to get user's assigned team
	suite.userRepo.EXPECT().GetByEmail(email).Return(member, nil)
	suite.teamRepo.EXPECT().GetByID(teamID).Return(team, nil)
...
// Tests for team validation logic in CreateDeployment

func (suite *AICoreServiceTestSuite) TestCreateDeployment_WithValidTeam_Success() {
	// Setup
	email := "team.member@example.com"
	teamID := uuid.New()
...
func (suite *AICoreServiceTestSuite) TestCreateDeployment_WithInvalidTeam_Error() {
	// Setup
	email := "team.member@example.com"
	teamID := uuid.New()
...
func (suite *AICoreServiceTestSuite) TestCreateDeployment_WithConfigurationRequest_ValidTeam_Success() {
	// Setup
	email := "team.member@example.com"
	teamID := uuid.New()
	...
	// Setup mocks - getUserAccessibleTeams is called multiple times for validation
	// CreateDeployment calls CreateConfiguration internally (1 call each), then validates team access again (1 call each), so total is 2 calls
	suite.userRepo.EXPECT().GetByEmail(email).Return(member, nil).Times(2) // 1 for config creation, 1 for deployment creation
	suite.teamRepo.EXPECT().GetByID(teamID).Return(team, nil).Times(2)
...
func (suite *AICoreServiceTestSuite) TestCreateDeployment_NoTeamSpecified_UsesUserTeam_Success() {
	// Setup
	email := "team.member@example.com"
	teamID := uuid.New()
...
func (suite *AICoreServiceTestSuite) TestCreateDeployment_TeamValidationFails_Error() {
	// Setup
	email := "nonexistent@example.com"
...
func (suite *AICoreServiceTestSuite) TestCreateDeployment_MultipleTeamsAccess_ValidTeam_Success() {
	// Setup - User with access to multiple teams via metadata
	email := "multi.team.user@example.com"
	teamID := uuid.New()

	metadata := map[string]interface{}{
		"ai_instances": []string{"team-alpha", "team-beta", "team-gamma"},
	}
	metadataJSON, _ := json.Marshal(metadata)
	...
	deploymentRequest := &service.AICoreDeploymentRequest{
		ConfigurationID: &configID,
		TTL:             "1h",
		Team:            "team-beta", // User has access to this team via metadata
	}
	...
	// Setup mocks - getUserAccessibleTeams returns multiple teams
	suite.userRepo.EXPECT().GetByEmail(email).Return(member, nil).Times(1) // Once for validation
	suite.teamRepo.EXPECT().GetByID(teamID).Return(team, nil).Times(1)
...
// Tests for the fallback scenario in GetMe where a manager doesn't own their current team's group
// and the system searches all organizations for a group they own

func (suite *AICoreServiceTestSuite) TestGetMe_Manager_FallbackToAllOrgsSearch_Success() {
	// Setup - Manager who doesn't own their current team's group but owns a group in another org
	username := "cross.org.manager"
	teamID := uuid.New()
	...
	// Current team and group (manager doesn't own this group)
	currentTeam := &models.Team{
		BaseModel: models.BaseModel{ID: teamID, Name: "current-team"},
		GroupID:   currentGroupID,
	}

	currentGroup := &models.Group{
		BaseModel: models.BaseModel{ID: currentGroupID, Name: "current-group"},
		Owner:     "someone.else", // Manager doesn't own this group
		OrgID:     currentOrgID,
	}

	// Target group in another org (manager owns this one)
	targetGroup := &models.Group{
		BaseModel: models.BaseModel{ID: targetGroupID, Name: "target-group"},
		Owner:     username, // Manager owns this group
		OrgID:     targetOrgID,
	}
	...
	currentOrg := &models.Organization{
		BaseModel: models.BaseModel{ID: currentOrgID, Name: "current-org"},
		Owner:     "org.owner",
	}

	targetOrg := &models.Organization{
		BaseModel: models.BaseModel{ID: targetOrgID, Name: "target-org"},
		Owner:     "another.owner",
	}

	// Teams in the target group (the one the manager owns)
	teamsInTargetGroup := []models.Team{
		{BaseModel: models.BaseModel{Name: "team-alpha"}},
		{BaseModel: models.BaseModel{Name: "team-beta"}},
		{BaseModel: models.BaseModel{Name: "team-gamma"}},
	}
	...
	// Setup mocks - this tests the fallback path where orgRepo.GetAll is called
	suite.userRepo.EXPECT().GetByName(username).Return(member, nil)
	suite.teamRepo.EXPECT().GetByID(teamID).Return(currentTeam, nil)
	suite.groupRepo.EXPECT().GetByID(currentGroupID).Return(currentGroup, nil)
	// First search in current org doesn't find owned group
	suite.groupRepo.EXPECT().GetByOrganizationID(currentOrgID, gomock.Any(), gomock.Any()).Return([]models.Group{*currentGroup}, int64(1), nil)
	// Fallback: search all orgs
	suite.orgRepo.EXPECT().GetAll(1000, 0).Return([]models.Organization{*currentOrg, *targetOrg}, int64(2), nil)
	// Search groups in current org (already searched above, but called again in fallback)
	suite.groupRepo.EXPECT().GetByOrganizationID(currentOrgID, gomock.Any(), gomock.Any()).Return([]models.Group{*currentGroup}, int64(1), nil)
	// Search groups in target org - finds the owned group
	suite.groupRepo.EXPECT().GetByOrganizationID(targetOrgID, gomock.Any(), gomock.Any()).Return([]models.Group{*targetGroup}, int64(1), nil)
	// Get teams in the owned group
	suite.teamRepo.EXPECT().GetByGroupID(targetGroupID, gomock.Any(), gomock.Any()).Return(teamsInTargetGroup, int64(len(teamsInTargetGroup)), nil)
...
func (suite *AICoreServiceTestSuite) TestGetMe_Manager_FallbackToAllOrgsSearch_NoOwnedGroupFound() {
	// Setup - Manager who doesn't own any group in any organization
	username := "no.group.manager"
	teamID := uuid.New()
	currentGroupID := uuid.New()
	currentOrgID := uuid.New()
	otherOrgID := uuid.New()
	...
	otherGroup := &models.Group{
		BaseModel: models.BaseModel{ID: uuid.New(), Name: "other-group"},
		Owner:     "another.person", // Manager doesn't own this group either
		OrgID:     otherOrgID,
	}
	...
	otherOrg := &models.Organization{
		BaseModel: models.BaseModel{ID: otherOrgID, Name: "other-org"},
		Owner:     "another.owner",
	}

	// Setup mocks - fallback search finds no owned groups, uses current team's group as final fallback
	suite.userRepo.EXPECT().GetByName(username).Return(member, nil)
	suite.teamRepo.EXPECT().GetByID(teamID).Return(currentTeam, nil)
	suite.groupRepo.EXPECT().GetByID(currentGroupID).Return(currentGroup, nil)
	// First search in current org doesn't find owned group
	suite.groupRepo.EXPECT().GetByOrganizationID(currentOrgID, gomock.Any(), gomock.Any()).Return([]models.Group{*currentGroup}, int64(1), nil)
	// Fallback: search all orgs
	suite.orgRepo.EXPECT().GetAll(1000, 0).Return([]models.Organization{*currentOrg, *otherOrg}, int64(2), nil)
	// Search groups in current org (no owned groups)
	suite.groupRepo.EXPECT().GetByOrganizationID(currentOrgID, gomock.Any(), gomock.Any()).Return([]models.Group{*currentGroup}, int64(1), nil)
	// Search groups in other org (no owned groups)
	suite.groupRepo.EXPECT().GetByOrganizationID(otherOrgID, gomock.Any(), gomock.Any()).Return([]models.Group{*otherGroup}, int64(1), nil)
	// Final fallback: use current team's group
	suite.teamRepo.EXPECT().GetByID(teamID).Return(currentTeam, nil)
	suite.groupRepo.EXPECT().GetByID(currentGroupID).Return(currentGroup, nil)
	// Get teams in current group (final fallback)
	teamsInCurrentGroup := []models.Team{
		{BaseModel: models.BaseModel{Name: "fallback-team"}},
	}
	suite.teamRepo.EXPECT().GetByGroupID(currentGroupID, gomock.Any(), gomock.Any()).Return(teamsInCurrentGroup, int64(1), nil)

	// Setup credentials
	suite.setupCredentials([]string{"fallback-team"})
	...
	suite.NotNil(result)
	suite.Equal(username, result.User)
	suite.Len(result.AIInstances, 1) // Only the fallback team
	suite.Contains(result.AIInstances, "fallback-team")
}

func (suite *AICoreServiceTestSuite) TestGetMe_Manager_FallbackToAllOrgsSearch_OrgRepoError() {
	// Setup - Manager where orgRepo.GetAll fails
	...
	// Setup mocks - orgRepo.GetAll returns error, should fall back to current team's group
	suite.userRepo.EXPECT().GetByName(username).Return(member, nil)
	suite.teamRepo.EXPECT().GetByID(teamID).Return(currentTeam, nil)
	suite.groupRepo.EXPECT().GetByID(currentGroupID).Return(currentGroup, nil)
	// First search in current org doesn't find owned group
	suite.groupRepo.EXPECT().GetByOrganizationID(currentOrgID, gomock.Any(), gomock.Any()).Return([]models.Group{*currentGroup}, int64(1), nil)
	// Fallback: search all orgs - returns error
	suite.orgRepo.EXPECT().GetAll(1000, 0).Return(nil, int64(0), fmt.Errorf("database error"))
	// Final fallback: use current team's group
	suite.teamRepo.EXPECT().GetByID(teamID).Return(currentTeam, nil)
	suite.groupRepo.EXPECT().GetByID(currentGroupID).Return(currentGroup, nil)
	// Get teams in current group
	teamsInCurrentGroup := []models.Team{
		{BaseModel: models.BaseModel{Name: "error-fallback-team"}},
	}
	suite.teamRepo.EXPECT().GetByGroupID(currentGroupID, gomock.Any(), gomock.Any()).Return(teamsInCurrentGroup, int64(1), nil)

	// Setup credentials
	suite.setupCredentials([]string{"error-fallback-team"})
	...
	suite.Equal(username, result.User)
	suite.Len(result.AIInstances, 1) // Only the fallback team
	suite.Contains(result.AIInstances, "error-fallback-team")
}

func (suite *AICoreServiceTestSuite) TestGetMe_Manager_FallbackToAllOrgsSearch_GroupRepoError() {
	// Setup - Manager where groupRepo.GetByOrganizationID fails during fallback search
	username := "group.error.manager"
	teamID := uuid.New()
	currentGroupID := uuid.New()
	currentOrgID := uuid.New()
	otherOrgID := uuid.New()
	...
	// Setup mocks - groupRepo.GetByOrganizationID fails for one org during fallback
	suite.userRepo.EXPECT().GetByName(username).Return(member, nil)
	suite.teamRepo.EXPECT().GetByID(teamID).Return(currentTeam, nil)
	suite.groupRepo.EXPECT().GetByID(currentGroupID).Return(currentGroup, nil)
	// First search in current org doesn't find owned group
	...
	// Search groups in current org (no owned groups)
	suite.groupRepo.EXPECT().GetByOrganizationID(currentOrgID, gomock.Any(), gomock.Any()).Return([]models.Group{*currentGroup}, int64(1), nil)
	// Search groups in other org - returns error
	suite.groupRepo.EXPECT().GetByOrganizationID(otherOrgID, gomock.Any(), gomock.Any()).Return(nil, int64(0), fmt.Errorf("group query error"))
	// Final fallback: use current team's group
	suite.teamRepo.EXPECT().GetByID(teamID).Return(currentTeam, nil)
	suite.groupRepo.EXPECT().GetByID(currentGroupID).Return(currentGroup, nil)
	// Get teams in current group
	teamsInCurrentGroup := []models.Team{
		{BaseModel: models.BaseModel{Name: "group-error-fallback-team"}},
	}
	suite.teamRepo.EXPECT().GetByGroupID(currentGroupID, gomock.Any(), gomock.Any()).Return(teamsInCurrentGroup, int64(1), nil)

	// Setup credentials
	suite.setupCredentials([]string{"group-error-fallback-team"})
	...
	suite.Equal(username, result.User)
	suite.Len(result.AIInstances, 1) // Only the fallback team
	suite.Contains(result.AIInstances, "group-error-fallback-team")
}

func (suite *AICoreServiceTestSuite) TestGetMe_Manager_FallbackToAllOrgsSearch_OrgRepoNil() {
	// Setup - Manager where orgRepo is nil (should skip fallback search)
	username := "nil.org.manager"
	teamID := uuid.New()
	currentGroupID := uuid.New()
	currentOrgID := uuid.New()
	...
	// Create service with nil orgRepo to test the nil check
	service := service.NewAICoreService(
		suite.userRepo,
		suite.teamRepo,
		suite.groupRepo,
		nil, // orgRepo is nil
	...
	// Setup mocks - should skip fallback search and use current team's group
	suite.userRepo.EXPECT().GetByName(username).Return(member, nil)
	suite.teamRepo.EXPECT().GetByID(teamID).Return(currentTeam, nil)
	suite.groupRepo.EXPECT().GetByID(currentGroupID).Return(currentGroup, nil)
	// First search in current org doesn't find owned group
	suite.groupRepo.EXPECT().GetByOrganizationID(currentOrgID, gomock.Any(), gomock.Any()).Return([]models.Group{*currentGroup}, int64(1), nil)
	// No fallback search because orgRepo is nil
	// Final fallback: use current team's group
	suite.teamRepo.EXPECT().GetByID(teamID).Return(currentTeam, nil)
	suite.groupRepo.EXPECT().GetByID(currentGroupID).Return(currentGroup, nil)
	// Get teams in current group
	teamsInCurrentGroup := []models.Team{
		{BaseModel: models.BaseModel{Name: "nil-org-fallback-team"}},
	}
	suite.teamRepo.EXPECT().GetByGroupID(currentGroupID, gomock.Any(), gomock.Any()).Return(teamsInCurrentGroup, int64(1), nil)

	// Setup credentials
	suite.setupCredentials([]string{"nil-org-fallback-team"})

	// Execute
	c := suite.createGinContext("")
	c.Set("username", username)
	result, err := service.GetMe(c)
	...
	suite.Equal(username, result.User)
	suite.Len(result.AIInstances, 1) // Only the fallback team
	suite.Contains(result.AIInstances, "nil-org-fallback-team")
}

func (suite *AICoreServiceTestSuite) TestGetMe_Manager_FallbackToAllOrgsSearch_MultipleOrgsWithOwnedGroup() {
	// Setup - Manager who owns groups in multiple organizations (should find first one and break)
	username := "multi.org.manager"
	teamID := uuid.New()
	currentGroupID := uuid.New()
	currentOrgID := uuid.New()
	firstTargetOrgID := uuid.New()
	secondTargetOrgID := uuid.New()
	firstTargetGroupID := uuid.New()
	// secondTargetGroupID := uuid.New() // Not used since we break after finding first group
	...
	// First target group (should be found first)
	firstTargetGroup := &models.Group{
		BaseModel: models.BaseModel{ID: firstTargetGroupID, Name: "first-target-group"},
		Owner:     username, // Manager owns this group
		OrgID:     firstTargetOrgID,
	}

	// Second target group (should not be reached due to break)
	// secondTargetGroup := &models.Group{
	// 	BaseModel: models.BaseModel{ID: secondTargetGroupID, Name: "second-target-group"},
	// 	Owner:     username, // Manager also owns this group
	// 	OrgID:     secondTargetOrgID,
	// }
	...
	firstTargetOrg := &models.Organization{
		BaseModel: models.BaseModel{ID: firstTargetOrgID, Name: "first-target-org"},
		Owner:     "first.owner",
	}

	secondTargetOrg := &models.Organization{
		BaseModel: models.BaseModel{ID: secondTargetOrgID, Name: "second-target-org"},
		Owner:     "second.owner",
	}

	// Teams in the first target group (the one that should be found)
	teamsInFirstTargetGroup := []models.Team{
		{BaseModel: models.BaseModel{Name: "first-team-alpha"}},
		{BaseModel: models.BaseModel{Name: "first-team-beta"}},
	}

	// Setup credentials
	suite.setupCredentials([]string{"first-team-alpha", "first-team-beta"})

	// Setup mocks - should find first owned group and break, not search second org
	suite.userRepo.EXPECT().GetByName(username).Return(member, nil)
	suite.teamRepo.EXPECT().GetByID(teamID).Return(currentTeam, nil)
	suite.groupRepo.EXPECT().GetByID(currentGroupID).Return(currentGroup, nil)
	// First search in current org doesn't find owned group
	suite.groupRepo.EXPECT().GetByOrganizationID(currentOrgID, gomock.Any(), gomock.Any()).Return([]models.Group{*currentGroup}, int64(1), nil)
	// Fallback: search all orgs
	suite.orgRepo.EXPECT().GetAll(1000, 0).Return([]models.Organization{*currentOrg, *firstTargetOrg, *secondTargetOrg}, int64(3), nil)
	// Search groups in current org (no owned groups)
	suite.groupRepo.EXPECT().GetByOrganizationID(currentOrgID, gomock.Any(), gomock.Any()).Return([]models.Group{*currentGroup}, int64(1), nil)
	// Search groups in first target org - finds owned group, should break here
	suite.groupRepo.EXPECT().GetByOrganizationID(firstTargetOrgID, gomock.Any(), gomock.Any()).Return([]models.Group{*firstTargetGroup}, int64(1), nil)
	// Should NOT search second target org due to break
	// Get teams in the first owned group
	suite.teamRepo.EXPECT().GetByGroupID(firstTargetGroupID, gomock.Any(), gomock.Any()).Return(teamsInFirstTargetGroup, int64(len(teamsInFirstTargetGroup)), nil)
	...
	suite.NotNil(result)
	suite.Equal(username, result.User)
	suite.Len(result.AIInstances, 2) // Teams from first owned group only
	suite.Contains(result.AIInstances, "first-team-alpha")
	suite.Contains(result.AIInstances, "first-team-beta")
}
