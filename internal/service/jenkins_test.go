...
func (suite *JenkinsServiceTestSuite) TearDownTest() {
	...
	os.Unsetenv("JENKINS_CFSSTATUS_JAAS_TOKEN")
}
...
// TestGetConcourseLatestExecution_Success tests successful latest execution retrieval
func (suite *JenkinsServiceTestSuite) TestGetConcourseLatestExecution_Success() {
	os.Setenv("JENKINS_CFSSTATUS_JAAS_TOKEN", "test-token-123")
	os.Setenv("JENKINS_P_USER", "test-user")

	suite.mockServer = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Verify the request path
		assert.Contains(suite.T(), r.URL.Path, "/job/concourse-status-developer-portal/lastSuccessfulBuild/api/json")
		assert.Equal(suite.T(), http.MethodGet, r.Method)
	...
	buildNumber, err := suite.jenkinsService.GetConcourseLatestExecution(context.Background())

	assert.NoError(suite.T(), err)
	assert.Equal(suite.T(), 42, buildNumber)
}

// TestGetConcourseLatestExecution_MissingCredentials tests missing credentials
func (suite *JenkinsServiceTestSuite) TestGetConcourseLatestExecution_MissingCredentials() {
	cfg := &config.Config{
		JenkinsBaseURL: "https://jenkins.example.com",
	}
	suite.jenkinsService = service.NewJenkinsService(cfg)
	...
	assert.Error(suite.T(), err)
	assert.Equal(suite.T(), 0, buildNumber)
	assert.Contains(suite.T(), err.Error(), "jenkins token not found")
}

// TestGetConcourseLatestExecution_NotFound tests 404 from Jenkins
func (suite *JenkinsServiceTestSuite) TestGetConcourseLatestExecution_NotFound() {
	os.Setenv("JENKINS_CFSSTATUS_JAAS_TOKEN", "test-token-123")
	os.Setenv("JENKINS_P_USER", "test-user")
	...
	assert.Error(suite.T(), err)
	assert.Equal(suite.T(), 0, buildNumber)
	assert.Contains(suite.T(), err.Error(), "jenkins request failed")
}

// TestGetConcourseJobs_Success tests successful jobs retrieval
func (suite *JenkinsServiceTestSuite) TestGetConcourseJobs_Success() {
	os.Setenv("JENKINS_CFSSTATUS_JAAS_TOKEN", "test-token-123")
	os.Setenv("JENKINS_P_USER", "test-user")

	suite.mockServer = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Verify the request path
		assert.Contains(suite.T(), r.URL.Path, "/job/concourse-status-developer-portal/42/artifact/concourse_jobs.json")
		assert.Equal(suite.T(), http.MethodGet, r.Method)
		...
		// Return mock response with jobs organized by landscape
		response := map[string][]interface{}{
			"cf-ae01": {
				map[string]interface{}{"name": "job1", "status": "success"},
				map[string]interface{}{"name": "job2", "status": "failed"},
			},
			"cf-eu10": {
				map[string]interface{}{"name": "job3", "status": "success"},
			},
		}
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(response)
	...
	result, err := suite.jenkinsService.GetConcourseJobs(context.Background(), 42, "all")
	...
	resultMap, ok := result.(map[string][]interface{})
	require.True(suite.T(), ok, "Result should be a map")
	assert.Contains(suite.T(), resultMap, "cf-ae01")
	assert.Contains(suite.T(), resultMap, "cf-eu10")
	assert.Len(suite.T(), resultMap["cf-ae01"], 2)
	assert.Len(suite.T(), resultMap["cf-eu10"], 1)
}

// TestGetConcourseJobs_WithLandscapeFilter tests filtering by specific landscape
func (suite *JenkinsServiceTestSuite) TestGetConcourseJobs_WithLandscapeFilter() {
	os.Setenv("JENKINS_CFSSTATUS_JAAS_TOKEN", "test-token-123")
	os.Setenv("JENKINS_P_USER", "test-user")
	...
	result, err := suite.jenkinsService.GetConcourseJobs(context.Background(), 42, "cf-ae01")

	assert.NoError(suite.T(), err)
	assert.NotNil(suite.T(), result)

	// Verify only the requested landscape is returned
	resultMap, ok := result.(map[string][]interface{})
	require.True(suite.T(), ok, "Result should be a map")
	assert.Contains(suite.T(), resultMap, "cf-ae01")
	assert.NotContains(suite.T(), resultMap, "cf-eu10")
	assert.Len(suite.T(), resultMap["cf-ae01"], 2)
}

// TestGetConcourseJobs_NonExistentLandscape tests filtering by non-existent landscape
func (suite *JenkinsServiceTestSuite) TestGetConcourseJobs_NonExistentLandscape() {
	os.Setenv("JENKINS_CFSSTATUS_JAAS_TOKEN", "test-token-123")
	os.Setenv("JENKINS_P_USER", "test-user")
	...
	result, err := suite.jenkinsService.GetConcourseJobs(context.Background(), 42, "non-existent")

	assert.NoError(suite.T(), err)
	assert.NotNil(suite.T(), result)

	// Verify empty result for non-existent landscape
	resultMap, ok := result.(map[string][]interface{})
	require.True(suite.T(), ok, "Result should be a map")
	assert.Empty(suite.T(), resultMap)
}

// TestGetConcourseJobs_MissingCredentials tests missing credentials
func (suite *JenkinsServiceTestSuite) TestGetConcourseJobs_MissingCredentials() {
	cfg := &config.Config{
		JenkinsBaseURL: "https://jenkins.example.com",
	}
	suite.jenkinsService = service.NewJenkinsService(cfg)
...
// TestGetConcourseJobs_NotFound tests 404 from Jenkins
func (suite *JenkinsServiceTestSuite) TestGetConcourseJobs_NotFound() {
	os.Setenv("JENKINS_CFSSTATUS_JAAS_TOKEN", "test-token-123")
	os.Setenv("JENKINS_P_USER", "test-user")
	...
	assert.Error(suite.T(), err)
	assert.Nil(suite.T(), result)
	assert.Contains(suite.T(), err.Error(), "jenkins artifact request failed")
}

// TestGetConcourseJobs_InvalidJSON tests invalid JSON response
func (suite *JenkinsServiceTestSuite) TestGetConcourseJobs_InvalidJSON() {
	os.Setenv("JENKINS_CFSSTATUS_JAAS_TOKEN", "test-token-123")
	os.Setenv("JENKINS_P_USER", "test-user")
	...
	assert.Error(suite.T(), err)
	assert.Nil(suite.T(), result)
	assert.Contains(suite.T(), err.Error(), "failed to parse concourse jobs")
}
