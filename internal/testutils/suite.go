...
// CreateAndSaveCategory creates a category with custom details and saves it to the database
func (s *BaseTestSuite) CreateAndSaveCategory(name, title, icon, color string) *models.Category {
	factories := NewFactorySet()
	category := factories.Category.CreateWithDetails(name, title, icon, color)
	err := s.DB.Create(category).Error
	if err != nil {
		s.T().Fatalf("Failed to create test category: %v", err)
	}
	return category
}

// CreateAndSaveLink creates a link with custom details and saves it to the database
func (s *BaseTestSuite) CreateAndSaveLink(owner uuid.UUID, categoryID uuid.UUID, title, url, tags string) *models.Link {
	factories := NewFactorySet()
	link := factories.Link.CreateWithDetails(owner, categoryID, title, url, tags)
	err := s.DB.Create(link).Error
	if err != nil {
		s.T().Fatalf("Failed to create test link: %v", err)
	}
	return link
}
...
func initSharedPGContainer() error {
	...
	resource, err := pool.RunWithOptions(&dockertest.RunOptions{
		Repository: "postgres",
		Tag:        "17-alpine", // Use same Postgres version as production: developer-portal-backend/docker/docker-compose.yml
		Env: []string{
