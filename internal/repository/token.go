...
// GetTokenByRefreshToken retrieves a token record by its refresh token value.
func (r *TokenRepository) GetByRefreshToken(refreshToken string) (*models.Token, error) {
	var tok models.Token
	if err := r.db.Where("provider = ? AND token = ?", constants.RefreshToken, refreshToken).First(&tok).Error; err != nil {
		return nil, err
	}
	return &tok, nil
}

// UpsertRefreshToken creates or updates a refresh token record for a given user.
// Implemented as a single-statement UPSERT to avoid race conditions
func (r *TokenRepository) UpsertRefreshToken(userUUID uuid.UUID, token string, expiresAt time.Time) error {
	tok := &models.Token{
		UserUUID:  userUUID,
