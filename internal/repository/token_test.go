...
func (suite *TokenRepositoryTestSuite) TestUpsertRefreshToken_CreateAndUpdate() {
	user := uuid.New()
	rt1 := "rtok-1"
	exp1 := time.Now().Add(30 * time.Minute)

	// Create new refresh token (stored as provided, no encryption)
	err := suite.repo.UpsertRefreshToken(user, rt1, exp1)
	suite.NoError(err)

	var rec1 models.Token
	err = suite.baseTestSuite.DB.Where("user_uuid = ?", user).First(&rec1).Error
	suite.NoError(err)
	suite.Equal(rt1, rec1.Token)
	suite.WithinDuration(exp1, rec1.ExpiresAt, 2*time.Second)

	// Update existing refresh token
	rt2 := "rtok-2"
	exp2 := time.Now().Add(90 * time.Minute)
	err = suite.repo.UpsertRefreshToken(user, rt2, exp2)
	suite.NoError(err)

	var rec2 models.Token
	err = suite.baseTestSuite.DB.Where("user_uuid = ?", user).First(&rec2).Error
	suite.NoError(err)
	suite.Equal(rt2, rec2.Token)
	suite.WithinDuration(exp2, rec2.ExpiresAt, 2*time.Second)
}

func (suite *TokenRepositoryTestSuite) TestGetByRefreshToken_Found() {
	user := uuid.New()
	refresh := "hashed-refresh-abc"
	exp := time.Now().Add(1 * time.Hour)

	// Create a refresh token via repository to ensure proper provider is set
	suite.Require().NoError(suite.repo.UpsertRefreshToken(user, refresh, exp))

	// Fetch by refresh token
	got, err := suite.repo.GetByRefreshToken(refresh)
	suite.NoError(err)
	suite.NotNil(got)
	suite.Equal(user, got.UserUUID)
	suite.Equal(refresh, got.Token)
}

func (suite *TokenRepositoryTestSuite) TestGetByRefreshToken_NotFound() {
	// Ensure table is empty
	suite.truncateTokens()

	_, err := suite.repo.GetByRefreshToken("does-not-exist")
	suite.Error(err)
	suite.Equal(gorm.ErrRecordNotFound, err)
}
