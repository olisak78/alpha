...
//   - {landscape_subdomain}  -> landscape metadata["subdomain"] (string; empty if missing)
//   - {health_suffix}        -> component metadata["health_suffix"] (string; empty if missing)
//   - {subdomain}            -> component metadata["subdomain"] (string; omitted when empty)
...
func BuildComponentHealthURL(
	...
	landscapeSubdomain := "" // default for {landscape_subdomain} is empty
	...
	// Extract landscape subdomain from landscape metadata
	if len(landscape.Metadata) > 0 {
		var landscapeMeta map[string]interface{}
		if err := json.Unmarshal(landscape.Metadata, &landscapeMeta); err == nil {
			if lsRaw, ok := landscapeMeta["subdomain"]; ok {
				if lsStr, ok := lsRaw.(string); ok && lsStr != "" {
					landscapeSubdomain = lsStr
				}
			}
		}
	}
	...
	t = strings.ReplaceAll(t, "{landscape_subdomain}", landscapeSubdomain)
