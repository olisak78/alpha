...
func (h *AlertsHandler) GetAlerts(c *gin.Context) {
	...
	alerts, err := h.alertsService.GetProjectAlerts(c.Request.Context(), projectID, userUUID, provider)
	if err != nil {
...
func (h *AlertsHandler) CreateAlertPR(c *gin.Context) {
	...
	prURL, err := h.alertsService.CreateAlertPR(c.Request.Context(), projectID, userUUID, provider, payload.FileName, payload.Content, payload.Message, payload.Description)
	if err != nil {
		if err.Error() == apperrors.ErrProjectNotFound.Error() {
			c.JSON(http.StatusNotFound, gin.H{"error": apperrors.ErrProjectNotFound.Error()})
			return
