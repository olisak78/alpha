...
func (h *PluginHandler) GetAllPlugins(c *gin.Context) {
	...
	if subscribed {
		...
		allPlugins, err := h.pluginService.GetAllPluginsWithViewer(limit, offset, userName)
		if err != nil {
		...
		if userName == "" {
			...
			plugins, err = h.pluginService.GetAllPluginsWithViewer(limit, offset, userName)
		}
	}
...
func (h *PluginHandler) GetPluginUI(c *gin.Context) {
	...
	uiContent, err := h.pluginService.GetPluginUIContent(c.Request.Context(), id, h.githubService, userUUID, provider)
	if err != nil {
		// Check if it's a "not found" error
		if err.Error() == "record not found" {
			c.JSON(http.StatusNotFound, gin.H{"error": "Plugin not found"})
