...
func (suite *AICoreHandlerTestSuite) TestDeleteDeployment_MissingDeploymentID() {
	// Execute - empty deployment ID
	req := httptest.NewRequest("DELETE", "/ai-core/deployments/", nil)
	w := httptest.NewRecorder()
	suite.router.ServeHTTP(w, req)

	// Assert - Gin returns 404 for missing path parameters
	suite.Equal(http.StatusNotFound, w.Code)
}

func (suite *AICoreHandlerTestSuite) TestDeleteDeployment_EmptyDeploymentID() {
	// This test covers the case where deploymentID is an empty string
	// We need to mock the route to pass an empty string as the parameter
	router := gin.New()
	router.DELETE("/ai-core/deployments/:deploymentId", func(c *gin.Context) {
		// Simulate empty deploymentId parameter
		c.Params = gin.Params{{Key: "deploymentId", Value: ""}}
		suite.handler.DeleteDeployment(c)
	})

	// Execute
	req := httptest.NewRequest("DELETE", "/ai-core/deployments/empty", nil)
	w := httptest.NewRecorder()
	...
	// ErrMissingDeploymentID is a ValidationError struct, so it gets serialized as an object
	errorObj, ok := response["error"].(map[string]interface{})
	suite.True(ok, "Error should be an object")
	suite.Equal("deployment", errorObj["Field"])
	suite.Equal("deploymentId parameter is required", errorObj["Message"])
}

func (suite *AICoreHandlerTestSuite) TestDeleteDeployment_NotFound() {
	// Setup
	deploymentID := "nonexistent-deployment"
	suite.aicoreService.EXPECT().DeleteDeployment(gomock.Any(), deploymentID).Return(nil, errors.ErrAICoreDeploymentNotFound)
...
func (suite *AICoreHandlerTestSuite) TestDeleteDeployment_AuthenticationError() {
	// Setup
	deploymentID := "deployment-1"
	suite.aicoreService.EXPECT().DeleteDeployment(gomock.Any(), deploymentID).Return(nil, errors.ErrUserEmailNotFound)
	...
	err := json.Unmarshal(w.Body.Bytes(), &response)
	suite.NoError(err)
	suite.Equal(errors.ErrAuthenticationRequired.Error(), response["error"])
}

func (suite *AICoreHandlerTestSuite) TestDeleteDeployment_AuthorizationError() {
	// Setup
	deploymentID := "deployment-1"
	suite.aicoreService.EXPECT().DeleteDeployment(gomock.Any(), deploymentID).Return(nil, errors.ErrUserNotAssignedToTeam)
...
func (suite *AICoreHandlerTestSuite) TestDeleteDeployment_CredentialsError() {
	// Setup
	deploymentID := "deployment-1"
	credentialsError := errors.NewAICoreCredentialsNotFoundError("team-alpha")
	suite.aicoreService.EXPECT().DeleteDeployment(gomock.Any(), deploymentID).Return(nil, credentialsError)
...
func (suite *AICoreHandlerTestSuite) TestDeleteDeployment_InternalServerError() {
	// Setup
	deploymentID := "deployment-1"
	suite.aicoreService.EXPECT().DeleteDeployment(gomock.Any(), deploymentID).Return(nil, errors.ErrInternalError)
