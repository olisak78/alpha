...
// mockJenkinsService is a simple mock implementation for testing without gomock
type mockJenkinsService struct {
	GetConcourseLatestExecutionFunc func(ctx context.Context) (int, error)
	GetConcourseJobsFunc            func(ctx context.Context, buildNumber int, landscape string) (interface{}, error)
}
...
func (m *mockJenkinsService) GetConcourseLatestExecution(ctx context.Context) (int, error) {
	if m.GetConcourseLatestExecutionFunc != nil {
		return m.GetConcourseLatestExecutionFunc(ctx)
	}
	return 0, nil
}

func (m *mockJenkinsService) GetConcourseJobs(ctx context.Context, buildNumber int, landscape string) (interface{}, error) {
	if m.GetConcourseJobsFunc != nil {
		return m.GetConcourseJobsFunc(ctx, buildNumber, landscape)
	}
	return nil, nil
}

// TestGetConcourseLatestExecution_Success tests successful latest execution retrieval
func TestGetConcourseLatestExecution_Success(t *testing.T) {
	gin.SetMode(gin.TestMode)

	mockSvc := &mockJenkinsService{
		GetConcourseLatestExecutionFunc: func(ctx context.Context) (int, error) {
			return 42, nil
		},
	}
	...
	handler := handlers.NewJenkinsHandler(mockSvc)
	router := gin.New()
	router.GET("/concourse/latest-execution", handler.GetConcourseLatestExecution)

	req, _ := http.NewRequest(http.MethodGet, "/concourse/latest-execution", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
	...
	assert.Equal(t, float64(42), response["buildNumber"]) // JSON numbers are float64
}

// TestGetConcourseLatestExecution_ServiceError tests service error handling
func TestGetConcourseLatestExecution_ServiceError(t *testing.T) {
	gin.SetMode(gin.TestMode)

	mockSvc := &mockJenkinsService{
		GetConcourseLatestExecutionFunc: func(ctx context.Context) (int, error) {
			return 0, fmt.Errorf("jenkins request failed")
		},
	}
	...
	err := json.Unmarshal(w.Body.Bytes(), &response)
	assert.NoError(t, err)
	assert.Contains(t, response["error"], "failed to get latest execution")
}

// TestGetConcourseJobs_Success tests successful concourse jobs retrieval
func TestGetConcourseJobs_Success(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	handler := handlers.NewJenkinsHandler(mockSvc)
	router := gin.New()
	router.GET("/concourse/jobs", handler.GetConcourseJobs)

	req, _ := http.NewRequest(http.MethodGet, "/concourse/jobs", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
	...
	err := json.Unmarshal(w.Body.Bytes(), &response)
	assert.NoError(t, err)
	assert.Contains(t, response, "cf-ae01")
}

// TestGetConcourseJobs_WithLandscapeFilter tests filtering by landscape
func TestGetConcourseJobs_WithLandscapeFilter(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	mockSvc := &mockJenkinsService{
		...
		GetConcourseJobsFunc: func(ctx context.Context, buildNumber int, landscape string) (interface{}, error) {
			assert.Equal(t, 42, buildNumber)
			assert.Equal(t, "cf-ae01", landscape)
			return map[string]interface{}{
				"cf-ae01": []interface{}{
					map[string]interface{}{"name": "job1", "status": "success"},
				},
	...
	req, _ := http.NewRequest(http.MethodGet, "/concourse/jobs?landscape=cf-ae01", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
...
// TestGetConcourseJobs_LatestExecutionError tests when latest execution fails
func TestGetConcourseJobs_LatestExecutionError(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
// TestGetConcourseJobs_GetJobsError tests when getting jobs fails
func TestGetConcourseJobs_GetJobsError(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	err := json.Unmarshal(w.Body.Bytes(), &response)
	assert.NoError(t, err)
	assert.Contains(t, response["error"], "failed to get concourse jobs")
}
