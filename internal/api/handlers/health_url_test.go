...
func TestBuildComponentHealthURL_TemplateWithLandscapeSubdomain(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	componentSvc := mocks.NewMockComponentServiceInterface(ctrl)
	landscapeSvc := mocks.NewMockLandscapeServiceInterface(ctrl)
	projectSvc := stubProjectService{tmpl: "https://{landscape_subdomain}{subdomain}{landscape_domain}/health", regex: "OK"}
	...
	landscapeMetadata := json.RawMessage(`{"subdomain":"dev."}`)
	landscape := &service.LandscapeResponse{
		ID:       landscapeID,
		Domain:   "developer-portal.cfs.c.eu-de-2.cloud.sap",
	...
	url, successRegex, err := BuildComponentHealthURL(componentSvc, landscapeSvc, projectSvc, componentID, landscapeID)
	assert.NoError(t, err)
	assert.Equal(t, "https://devdeveloper-portal.cfs.c.eu-de-2.cloud.sap/health", url)
	assert.Equal(t, "OK", successRegex)
}

func TestBuildComponentHealthURL_TemplateWithLandscapeSubdomainAndComponentSubdomain(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	url, successRegex, err := BuildComponentHealthURL(componentSvc, landscapeSvc, projectSvc, componentID, landscapeID)
	assert.NoError(t, err)
	assert.Equal(t, "https://dev.api.developer-portal.cfs.c.eu-de-2.cloud.sap/health", url)
	assert.Equal(t, "OK", successRegex)
}
