...
func (a *userRepoAdapter) GetByUUID(uuid uuid.UUID) (*models.User, error) {
	return a.repo.GetByID(uuid)
}
...
func SetupRoutes(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	if err != nil {
		log.Fatalf("Fatal: Failed to load auth config: %v", err)
	}

	var authHandler *auth.AuthHandler
	var authMiddleware *auth.AuthMiddleware
	var authService *auth.AuthService
	var githubAuthService service.GitHubAuthService
	if authConfig != nil {
		...
		if err != nil {
			...
			githubAuthService = service.NewAuthServiceAdapter(authService)
		}
	}

	docService := service.NewDocumentationService(docRepo, teamRepo, validator, githubAuthService)
	...
	// Initialize alert storage client and service
	alertStorageClient := client.NewAlertStorageClient(cfg.AlertStorageServiceUrl)
	alertStorageService := service.NewAlertStorageService(alertStorageClient)
	...
	alertStorageHandler := handlers.NewAlertStorageHandler(alertStorageService)
	...
	if authMiddleware == nil {
		log.Fatal("Authentication middleware is required but not initialized. Check auth configuration.")
	}
...

	{
		...
		// Ops2go proxy endpoints
		v1.GET("/cis-public/ops2go/centrallandscapes", healthHandler.ProxyOps2go)
		v1.GET("/cis-public/ops2go/globalaccounts", healthHandler.ProxyOps2goGlobalAccounts)
		...
		github := v1.Group("/github")
		{
			...
			github.DELETE("/repos/:owner/:repo/folders/*path", githubHandler.DeleteRepositoryFolder)
			// GitHub asset proxy for images and other assets
			github.GET("/asset", githubHandler.GetGitHubAsset)

			// GitHub routes with provider parameter{
			provider := github.Group("/:provider")
			{
				provider.GET("/pull-requests", githubHandler.GetMyPullRequests)
				...
				github.POST("/repos/:owner/:repo/contents/*path", githubHandler.CreateRepositoryFile)
				github.DELETE("/repos/:owner/:repo/contents/*path", githubHandler.DeleteRepositoryFile)
			}
		}
		// Sonar routes
		...
		// Concourse routes
		concourse := v1.Group("/concourse")
		{
			concourse.GET("/latest-execution", jenkinsHandler.GetConcourseLatestExecution)
			concourse.GET("/jobs", jenkinsHandler.GetConcourseJobs)
		}
		...
		// Alert storage routes
		alertStorage := v1.Group("/alert-storage")
		{
			alertStorage.GET("/projects", alertStorageHandler.GetAvailableProjects)                       // GET /api/v1/alert-storage/projects
			alertStorage.GET("/alerts/:project", alertStorageHandler.GetAlertsByProject)                  // GET /api/v1/alert-storage/alerts/:project?page=1&pageSize=50&severity=critical&status=firing
			alertStorage.GET("/alerts/:project/filters", alertStorageHandler.GetAlertFilters)             // GET /api/v1/alert-storage/alerts/:project/filters?severity=critical&landscape=production
			alertStorage.GET("/alerts/:project/:fingerprint", alertStorageHandler.GetAlertByFingerprint)  // GET /api/v1/alert-storage/alerts/:project/:fingerprint
			alertStorage.PUT("/alerts/:project/:fingerprint/label", alertStorageHandler.UpdateAlertLabel) // PUT /api/v1/alert-storage/alerts/:project/:fingerprint/label
		}
