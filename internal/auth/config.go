...
type AuthConfig struct {
	...
	JWTExpiresInMinutes       int                       `yaml:"jwt_expires_in_minutes" json:"jwt_expires_in_minutes"`
	GithubTokenExpiresInDays  int                       `yaml:"github_token_expires_in_days" json:"github_token_expires_in_days"`
	RefreshTokenExpiresInDays int                       `yaml:"refresh_token_expires_in_days" json:"refresh_token_expires_in_days"`
...
func LoadAuthConfig(configPath string) (*AuthConfig, error) {
	...
	// note duplication in code: default values are also set for k9s deployment in file /charts/developer-portal-backend/values.yaml
	config.JWTExpiresInMinutes = getIntFromEnv("JWT_EXPIRES_IN_MINUTES", 15)
	config.GithubTokenExpiresInDays = getIntFromEnv("GITHUB_TOKEN_EXPIRATION_IN_DAYS", 365)
	config.RefreshTokenExpiresInDays = getIntFromEnv("REFRESH_TOKEN_EXPIRATION_IN_DAYS", 14)
...
// getIntFromEnv retrieves an integer from environment variables or returns a default value
func getIntFromEnv(key string, def int) int {
	...
	if err != nil {
		log.Fatalf("invalid %s=%q (must be integer)", key, v)
	}
	return i
}
