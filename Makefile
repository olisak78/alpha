...
.PHONY: docker-push
docker-push: image-not-pushed ## Push Docker image to registry (use TAG=version to override)
...
.PHONY: image-not-pushed
image-not-pushed: ## Ensure image tag does not already exist in registry
	@echo "$(BLUE)Checking if image already exists in registry...$(NC)"
	@if docker buildx imagetools inspect $(IMAGE) > /dev/null 2>&1; then \
		echo "$(YELLOW)⚠️  Image already exists in registry: $(IMAGE). Not pushing.$(NC)"; \
		exit 1; \
	else \
		echo "$(BLUE)Image not found. Proceeding...$(NC)"; \
	fi

.PHONY: git-tag
git-tag: ## Create and push git tag for frontend image
	@echo "$(BLUE)Creating git tag for frontend release...$(NC)"
	@echo "  Using TAG: $(TAG)"
	@GIT_TAG="frontend-$(TAG)"; \
	if git rev-parse -q --verify "refs/tags/$$GIT_TAG" >/dev/null; then \
		echo "$(YELLOW)⚠️  Git tag already exists: $$GIT_TAG. Skipping creation.$(NC)"; \
	else \
		git tag -a "$$GIT_TAG" -m "Release frontend $(TAG)"; \
		echo "$(GREEN)✅ Created git tag: $$GIT_TAG$(NC)"; \
	fi; \
	if git push origin "$$GIT_TAG"; then \
		echo "$(GREEN)✅ Pushed git tag: $$GIT_TAG$(NC)"; \
	else \
		echo "$(YELLOW)⚠️  Failed to push tag $$GIT_TAG. Please push manually.$(NC)"; \
	fi
...
.PHONY: lint-sarif
lint-sarif: ## Generate ESLint SARIF report (outputs eslint-results.sarif)
	@npm run get
	@npm run lint_sarif
	@echo "$(GREEN)✅ Generated SARIF: eslint-results.sarif$(NC)"
